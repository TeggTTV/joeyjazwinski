...
import { ArrowUpRight, Mail, MessageSquare, Clock, ShieldCheck, Sparkles } from 'lucide-react';
...
const ContactSection: React.FC = () => {
	return (
		<section className="w-full py-28 px-4 sm:px-6 md:px-8 bg-background relative overflow-hidden">
			...
			<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[650px] h-[450px] bg-[radial-gradient(ellipse_at_center,rgba(99,102,241,0.12),transparent_70%)] pointer-events-none blur-3xl" />
			...
			<div className="max-w-4xl mx-auto relative z-10">
				...
				<motion.div
					...
					className="p-1.5 rounded-[2.5rem] bg-white/[0.03] dark:bg-white/[0.02] border border-white/10 shadow-2xl backdrop-blur-2xl"
				>
					<div className="p-8 sm:p-12 md:p-16 rounded-[calc(2.5rem-0.375rem)] bg-card/95 backdrop-blur-2xl border border-white/5 shadow-[inset_0_1px_1px_rgba(255,255,255,0.08)] text-center flex flex-col items-center">
						...
						<p className="text-sm sm:text-base md:text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed mb-10 font-normal">
							Whether you want to discuss a full-stack project, explore technical partnerships, or collaborate on curriculum, my inbox is open.
						</p>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md border-t border-border/40 pt-8">
							<div className="flex items-center justify-center gap-2.5 px-4 py-3 rounded-2xl bg-white/[0.02] border border-white/5 text-xs text-muted-foreground">
							...
							<div className="flex items-center justify-center gap-2.5 px-4 py-3 rounded-2xl bg-white/[0.02] border border-white/5 text-xs text-muted-foreground">
								...
								<span className="text-foreground font-medium">Open to select contracts</span>
							</div>
						</div>
					</div>
				</motion.div>
