	...
	useEffect(() => {
		...
		try {
			...
			const generateTypes = (
				obj: any,
				name: string,
			): { types: string; interfaces: string[] } => {
				...
				if (typeof obj === 'object') {
					...
					keys.forEach((key) => {
						const val = obj[key];
						const cleanKey = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(key)
							? key
							: `"${key}"`;
					...
					return {
						types: name,
						interfaces: [...subInterfaces, currentInterface],
					};
				}
			...
			const generateZod = (
				...
				if (Array.isArray(obj)) {
					...
					const baseSchema =
						itemSchemas.size > 0
							? Array.from(itemSchemas).join(' || ')
							: 'z.any()';
					return {
						zod: `z.array(${baseSchema})`,
						schemas: subSchemas,
					};
				}
				if (typeof obj === 'object') {
					...
					return {
						zod: schemaVarName,
						schemas: [...subSchemas, currentSchema],
					};
				}
				if (typeof obj === 'string')
					return { zod: 'z.string()', schemas: [] };
				if (typeof obj === 'number')
					return { zod: 'z.number()', schemas: [] };
				if (typeof obj === 'boolean')
					return { zod: 'z.boolean()', schemas: [] };
