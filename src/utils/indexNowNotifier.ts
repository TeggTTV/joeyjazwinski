...
export async function notifyIndexNow(blogUrl: string): Promise<void> {
  const promises = SEARCH_ENGINES.map((engine) => {
    const apiUrl = `${engine}/indexnow?url=${encodeURIComponent(blogUrl)}&key=${INDEX_NOW_KEY}`;
    return fetch(apiUrl, { method: 'GET' })
      .then((response) => {
        if (!response.ok) {
          console.error(`Failed to notify ${engine}:`, response.statusText);
        } else {
          console.log(`Successfully notified ${engine} about ${blogUrl}`);
        }
      })
      .catch((error) => {
