...
// Update lessons with courseId dynamically
...
for (const lesson of lessons) {
    if (lesson.courseSlug) {
        const course = await coursesCollection.findOne({ slug: lesson.courseSlug });
        if (course) {
            await lessonsCollection.updateOne(
                { _id: lesson._id },
                { $set: { courseId: course._id } }
            );
            print(`Updated lesson ${lesson.slug} with courseId ${course._id}`);
        } else {
            print(`Course with slug ${lesson.courseSlug} not found for lesson ${lesson.slug}`);
        }
    } else {
        print(`Lesson ${lesson.slug} does not have a courseSlug field`);
    }
}
