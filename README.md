...
- [ ] Set up MDX blog/tutorials system
- [ ] Build simple Commenting system (Giscus or placeholder)
- [ ] Add User Customization (localStorage)
...
{
  _id: ObjectId,
  pageId: String, // Link to blog/tutorial slug
  userId: String, // Anonymous ID or user email
  commentText: String,
  createdAt: Date
}
```
...
## 🚀 MVP Feature Roadmap(First Build Plan)

 | Week | Focus | Notes |
| ------| ----------------------------------------------| -----------------------------------------|
| 1 | Landing Page + Blog Index Page Layout | Pure static pages, design only |
| 2 | Dynamic Blog Slug Pages + SEO setup | `getStaticPaths`, `getStaticProps` |
| 3 | Tutorials Page + Language Selector | Same dynamic approach as blogs |
| 4 | Customize Page + Light / Dark + Accent Picker | Tailwind theme switching |
| 5 | MongoDB Setup(Blogs / Tutorials DB) | Connect and load via API Routes |
| 6 | Comment System(basic, unauthenticated) | MongoDB Insert, Fetch, Display |
| 7 | Auth Setup(optional) + User Dashboard | NextAuth, JWT session |
| 8 | Final Polish(Animations, Framer Motion) | Small UX touches, deploy final build |
